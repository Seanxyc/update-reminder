...
const getExternal = () => {
  const externalJSONPath = process.env.EXTERNAL_PATH?.trim()
    ? path.join(process.cwd(), process.env.EXTERNAL_PATH.trim())
    : ''
...
outputDir.forEach(val => {
  ...
  fs.writeFile(
    outputVersionPath,
    `{ "version": "${process.env.VERSION || packageJsonObject.version}", "external": ${getExternal()} }`,
    () => {
